...
const {computeAverageIntersectDistance, normalizeSuffixe} = require('./geo-intersect')
...
test('basic usage: match', t => {
  ...
  t.true(computeAverageIntersectDistance(v1, v2) >= 0)
})
...
test('basic usage: mismatch', t => {
  ...
  t.false(computeAverageIntersectDistance(v1, v2) === -1)
})

test('normalizeSuffixe', t => {
  t.is(normalizeSuffixe('bis'), 'B')
  t.is(normalizeSuffixe('a'), 'A')
  t.is(normalizeSuffixe('C'), 'C')
  t.is(normalizeSuffixe(null), '')
  t.is(normalizeSuffixe(), '')
})
