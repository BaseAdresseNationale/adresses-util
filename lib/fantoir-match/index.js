  ...
  return {
    findCodeVoie(nomVoie) {
      ...
      if (significantWordsCompareResults.length > 0) {
        ...
        return selectBestResult(significantWordsCompareResults, ctx).codeRivoli
      }
      ...
      if (phoneticCompareResults.length > 0) {
        stats.phonetic++
        return selectBestResult(phoneticCompareResults, ctx).codeRivoli
      }
      ...
      if (overlapCompareResults.length > 0) {
        stats.overlap++
        return selectBestResult(overlapCompareResults, ctx).codeRivoli
      }
